...
lv_obj_t *status_bar_create(lv_obj_t *parent) {
    ...
    lv_label_set_text(ac_count_label, "0");
    ...
    lv_obj_align(ac_count_label, LV_ALIGN_LEFT_MID, 30, 0);

    // Nav buttons (center) — 4 buttons at 42px wide
    int nav_total_w = 4 * 42 + 3 * 4;
    int nav_x0 = (LCD_H_RES - nav_total_w) / 2;
    for (int i = 0; i < 4; i++) {
        nav_btns[i] = lv_obj_create(bar);
        lv_obj_set_size(nav_btns[i], 42, 20);
        lv_obj_set_pos(nav_btns[i], nav_x0 + i * 46, 2);
        ...
        lv_obj_set_style_radius(nav_btns[i], 3, 0);
    ...
    lv_obj_set_pos(auto_label, nav_x0 + nav_total_w + 4, 5);
    ...
    lv_obj_align(gear_icon, LV_ALIGN_RIGHT_MID, -4, 0);
    lv_obj_add_flag(gear_icon, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_set_ext_click_area(gear_icon, 10);
    ...
    lv_obj_align(update_label, LV_ALIGN_RIGHT_MID, -24, 0);
...
void status_bar_update(bool wifi_connected, int aircraft_count, uint32_t last_update_ms) {
    ...
    } else if (net == NET_ETHERNET) {
        lv_label_set_text(wifi_icon, "E");
        lv_obj_set_style_text_color(wifi_icon, STATUS_ACCENT_COLOR, 0);
    } else {
        lv_label_set_text(wifi_icon, "--");
    ...
    lv_label_set_text_fmt(ac_count_label, "%d", aircraft_count);
